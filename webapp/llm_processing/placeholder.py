...
allrecords ::= (
  ...
  ws "\"doctorFirstName\":" ws string ","
  ws "\"doctorLastName\":" ws string ","
  ws "}"
)
...
# minimal canonical type mapping for placeholders
_FIELD2TYPE = {
    # names
    "patientFirstName": "NAME", "patientMiddleName": "NAME", "patientLastName": "NAME",
    "patientName": "NAME", "doctorFirstName": "NAME", "doctorLastName": "NAME", "patientHonorific": "NAME",

    # dates
    "patientBirthDate": "DATE", "admissionDate": "DATE", "dischargeDate": "DATE",
    "deathDate": "DATE", "otherDateElements": "DATE",

    # phones & email
    "patientPhoneNumber": "PHONE", "patientFaxNumber": "PHONE", "patientEmail": "EMAIL",

    # ids
    "patientSSN": "SSN", "patientMRN": "MRN", "healthPlanBeneficiaryNumber": "ID",
    "accountNumber": "ID", "certificateOrLicenseNumber": "ID", "patientID": "ID",
    "vehicleIdentifier": "ID", "vehicleVIN": "ID", "vehicleLicensePlate": "ID",
    "deviceIdentifier": "ID", "deviceSerialNumber": "ID", "otherUniqueIdOrCode": "ID",

    # address-like
    "patientAddress": "ADDRESS", "patientStreet": "ADDRESS", "patientHouseNumber": "ADDRESS",
    "patientCity": "ADDRESS", "patientCounty": "ADDRESS", "patientPrecinct": "ADDRESS",
    "patientPostalCode": "ADDRESS", "patientState": "ADDRESS", "geocode": "ADDRESS",
...
def _try_json_loads_with_patch(raw: str):
    try:
        ...
        # 1) 先去掉所有 '},' 或 '],' 之前的尾随逗号（JSON 不允许）
        s1 = _slice_outer_object(raw)
        s1 = re.sub(r',\s*([}\]])', r'\1', s1)  # ★ 新增：移除尾随逗号

        try:
            return json.loads(s1), "patched_trailing_commas"
        except Exception as e2:
            # 2) 再做你已有的 patientAge 修补（跨行/未闭合引号）
            patched = re.sub(
                r'"patientAge"\s*:\s*"(?:(?:\\.|[^"\\])*\n(?:.*?))?",',
                r'"patientAge": "",',
                s1,
            ...
            try:
                return json.loads(patched), "patched_trailing_commas+patientAge"
            except Exception as e3:
                return None, f"json_error: {e3}"
...
def _strip_honorifics(s: str) -> str:
    # 去前缀称谓 + 末尾标点
    x = s.strip()
    x = re.sub(r"^[\s,]*(?:" + "|".join(_HONORIFICS) + r")\.?\s+", "", x, flags=re.I)
    return x.strip(" ,.;:")

def _name_tokens(s: str):
    # 仅保留字母和点，拆分为词
    x = re.sub(r"[^A-Za-z.\s]", " ", s)
    toks = [t for t in re.split(r"\s+", x) if t]
    return toks

def _name_signature(s: str):
    """
    返回 (last_lower, first_initial_or_None)
    - "John M. Doe" -> ("doe","j")
    - "Mr. Doe"     -> ("doe", None)
    - "Doe"         -> ("doe", None)
    """
    s1 = _strip_honorifics(s)
    toks = _name_tokens(s1)
    if not toks:
        return (None, None)
    last = toks[-1].rstrip(".").lower()
    # 找第一个像“名”的 token（跳过单独的中间名首字母）
    first = None
    for t in toks[:-1]:
        t1 = t.rstrip(".")
        if len(t1) >= 2:  # 认为是完整名
            first = t1[0].lower()
            break
        # 全是首字母的情形（M.）不当作 first
    return (last, first)

def _name_sig_equiv(sig_new, sig_exist):
    """
    判定两个签名是否可视为同一人：
    - last 必须相同；
    - first_initial 相同，或其中之一为 None
    """
    (ln1, fi1), (ln2, fi2) = sig_new, sig_exist
...
def _enumerate_spans(report_text: str, surface: str):
    for m in re.finditer(re.escape(surface), report_text):
        yield (m.start(), m.end())


_TYPE_PRIORITY = {"NAME": 6, "ID": 5, "ADDRESS": 4, "DATE": 3, "PHONE": 2, "EMAIL": 2, "URL":1, "IP":1, "SSN":5, "MRN":5}

def _resolve_overlaps(cands):
    # cands: list of dicts with keys: start,end,surface,type,source_key
    cands = sorted(
        cands,
        key=lambda x: (-(x["end"]-x["start"]), x["start"], -_TYPE_PRIORITY.get(x["type"], 0))
    )
    kept = []
    last_ends = []
    for c in cands:
        conflict = False
        for k in kept:
            if not (c["end"] <= k["start"] or c["start"] >= k["end"]):
                conflict = True
                break
        if not conflict:
            kept.append(c)
    # 最终按start升序返回，便于后续编号与右→左替换
    return sorted(kept, key=lambda x: x["start"])

def _assign_placeholders_and_replace(report_text: str, spans_sorted):
    counters = {}
    enriched = []

    # 已见到的“人” -> 占位符；既存键为精细签名，也为 last-only 别名
    name_registry = {}   # key: (last, first_initial or None) -> ph

    for s in spans_sorted:  # 左→右（便于首见名成为 NAME_1）
        t = s["type"]
        if t != "NAME":
        ...
        # ---- NAME：尝试复用 ----
        sig = _name_signature(s["surface"])
        reuse_ph = None
        # 1) 直接命中完全相同的签名
        if sig in name_registry:
            reuse_ph = name_registry[sig]
        else:
            # 2) 与已登记的人做等价判断（支持 first_initial 缺失）
            for exist_sig, ph0 in name_registry.items():
                if _name_sig_equiv(sig, exist_sig):
                    reuse_ph = ph0
                    break
        ...
        if reuse_ph:
            ...
            s["ph"] = ph
            # 登记精细签名
            name_registry[sig] = ph
            # 再登记一个 last-only 的别名，便于后续 "Mr. Doe" 命中
            last, _fi = sig
            if last:
                name_registry[(last, None)] = ph

        enriched.append(s)

    # 右→左替换
    masked = report_text
    for s in sorted(enriched, key=lambda x: x["start"], reverse=True):
        masked = masked[:s["start"]] + s["ph"] + masked[s["end"]:]
    return masked, enriched
...
# ——电话字段的语义回收（和日期同理）——
        ...
        # === 占位符：汇总候选表面串（带类型来源）===
        candidates = []  # 每项: {"surface": str, "type": str, "source_key": str}

        # a) 来自 LLM 字段值
        for k, v in info_dict.items():
            if isinstance(v, str) and v.strip():
                t = _FIELD2TYPE.get(k)  # 例如 patientName -> NAME
                if t:
                    candidates.append({"surface": v, "type": t, "source_key": k})

        # b) 来自“语义回收”的原文表面串（日期/电话）
        for s in row_out.get("_verbatim_date_spans", []):
            if isinstance(s, str) and s.strip():
                candidates.append({"surface": s, "type": "DATE", "source_key": "_verbatim_date_spans"})
        for s in row_out.get("_verbatim_phone_spans", []):
            if isinstance(s, str) and s.strip():
                candidates.append({"surface": s, "type": "PHONE", "source_key": "_verbatim_phone_spans"})

        # c) 兜底（可选）：把所有在文中的“像日期/像电话”的片段也加入候选
        for s in row_out.get("_all_date_spans", []):
            if isinstance(s, str) and s.strip():
                candidates.append({"surface": s, "type": "DATE", "source_key": "_all_date_spans"})
        for s in row_out.get("_all_phone_spans", []):
            if isinstance(s, str) and s.strip():
                candidates.append({"surface": s, "type": "PHONE", "source_key": "_all_phone_spans"})

        # === 把候选映射为原文中的 spans（可能多处命中）===
        span_cands = []
        report_text = info["report"]
        for c in candidates:
            for (st, ed) in _enumerate_spans(report_text, c["surface"]):
                span_cands.append({
                    "start": st, "end": ed,
                    "surface": report_text[st:ed],
                    "type": c["type"],
                    "source_key": c["source_key"],
                })

        # === 重叠裁决（长优先/类型优先），并按 start 升序返回 ===
        spans = _resolve_overlaps(span_cands)

        # === 分配占位符并右→左替换，得到 masked 文本与带 ph 的 spans ===
        masked_text, enriched_spans = _assign_placeholders_and_replace(report_text, spans)

        # 输出占位符版本文本 + sidecar
        row_out["masked_report_placeholders"] = masked_text

        row_out["placeholder_sidecar"] = json.dumps({
            "doc_id": _id,
            "placeholders": [
                {
                    "ph": s["ph"],
                    ...
                    "attrs": {
                        ...
                        "name_signature": list(_name_signature(s["surface"])) if s["type"] == "NAME" else None
}
                }
    ...
    SPECIAL_TAKE_FIRST = {"masked_report_placeholders", "placeholder_sidecar"}

    def _agg_flat(col):
        # 文本原报告仍然拼接（如果你确实有分片报告要合并）
        if col.name == "report":
            return " ".join(col)
        # 占位符文本与 sidecar 取第一条（同一 base_id 多症状重复）
        if col.name in SPECIAL_TAKE_FIRST:
            for v in col:
                if isinstance(v, str) and v:
                    return v
            return ""
        # 其他列：把每个单元里的 list 拿出来连成一层
        flat = []
        for v in col:
    ...
    cols_to_exclude = {
        "id", "base_id", "report", "metadata",
        "masked_report_placeholders", "placeholder_sidecar",
        "_verbatim_date_spans", "_all_date_spans",
        "_verbatim_phone_spans", "_all_phone_spans"
    }
    ...
    aggregated_df["masked_report_legacy"] = aggregated_df.apply(
...
if __name__ == "__main__":
    ...
    try:
        ...
        out_csv = "llm_place_holder_output.csv"
        df_out.to_csv(out_csv, index=False)

 # 可选：把 placeholder_sidecar 单独导出为 JSONL（每行一条）
        # sidecar_path = "llm_output.sidecar.jsonl"
        # with open(sidecar_path, "w", encoding="utf-8") as fw:
        #     for s in df_out["placeholder_sidecar"]:
        #         if isinstance(s, str) and s.strip():
        #             fw.write(s.strip() + "\n")
